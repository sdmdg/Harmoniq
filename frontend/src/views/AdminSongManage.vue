  ...
  <div class="min-h-screen bg-black text-white p-6">
    ...
    <div class="mb-8">
      ...
      <p class="text-gray-400">
        Manage your music library with advanced filtering and classification
      </p>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div>
          ...
          >
            ...
            <option v-for="album in uniqueAlbums" :key="album" :value="album">
              {{ album }}
            </option>
          </select>
        </div>
        ...
        <div>
          ...
          >
            ...
            <option v-for="mood in uniqueMoods" :key="mood" :value="mood">
              {{ mood }}
            </option>
          </select>
        </div>
      ...
      >
        <div class="flex items-center gap-4">
          <span class="text-sm text-gray-400"
            >{{ filteredSongs.length }} songs found</span
          >
          <select
            v-model="limit"
    ...
    <div
      v-if="error"
      class="mb-6 p-4 bg-red-900/50 border border-red-700 rounded-lg text-red-200"
    >
      {{ error }}
    </div>
    <div
      v-if="success"
      class="mb-6 p-4 bg-green-900/50 border border-green-700 rounded-lg text-green-200"
    >
      {{ success }}
    </div>
    ...
    <!-- Songs Table -->
    <div
      class="bg-gray-800/50 rounded-xl border border-gray-800 overflow-hidden"
    >
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gray-800/50">
            <tr>
              <th class="text-left py-4 px-6 text-sm font-medium text-gray-300">
                Song
              </th>
              <th class="text-left py-4 px-6 text-sm font-medium text-gray-300">
                Album
              ...
              <th
                class="text-right py-4 px-6 text-sm font-medium text-gray-300"
              >
                Actions
              </th>
            </tr>
          ...
          <tbody>
            ...
            >
              ...
              <td class="py-4 px-6">
                <span
                  v-if="song.genre"
                  class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-900/50 text-blue-200 border border-blue-800"
                >
                  {{ song.genre }}
                </span>
              ...
              <td class="py-4 px-6">
                <span
                  v-if="song.mood"
                  class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-900/50 text-purple-200 border border-purple-800"
                >
                  {{ song.mood }}
                </span>
                <span v-else class="text-gray-500">—</span>
      ...
      <!-- Pagination -->
      <div
        class="border-t border-gray-800 px-6 py-4 flex items-center justify-between"
      >
        <div class="text-sm text-gray-400">
          Showing {{ (currentPage - 1) * limit + 1 }} to
          {{ Math.min(currentPage * limit, filteredSongs.length) }} of
...
const filteredSongs = computed(() => {
  ...
  if (selectedAlbum.value) {
    filtered = filtered.filter(
      (song) => song.album_name === selectedAlbum.value
    );
  }
...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(filteredSongs.value.length / limit.value))
);
...
function playRow(row) {
  ...
  // Allow playing songs with or without encryption keys
  const a = {
    id: "admin-all-songs",
    title: "All Songs",
    tracks: songs.value.map((r) => ({
