  ...
  <div class="min-h-screen bg-black text-white">
    ...
    <div class="border-b border-gray-800 bg-gray-900/50 backdrop-blur-sm">
      <div class="px-6 py-4">
        ...
        <p class="text-gray-400 text-sm mt-1">Manage all users and artists on your platform</p>
      </div>
    </div>
    ...
    <div class="p-6 space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
          ...
          <div class="text-2xl font-bold text-white">{{ users.filter(u => u.role === 'artist').length }}</div>
        </div>
        <div class="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
          <div class="text-gray-400 text-sm">Listeners</div>
          <div class="text-2xl font-bold text-white">{{ users.filter(u => u.role === 'listener').length }}</div>
        </div>
        <div class="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
          <div class="text-gray-400 text-sm">Active Today</div>
          <div class="text-2xl font-bold text-white">{{ Math.floor(total * 0.3) }}</div>
        </div>
      </div>
          ...
          <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center flex-1">
            ...
            <div class="flex gap-2">
              <button 
                ...
                :class="role === '' ? 'bg-green-600 border-green-600 text-white' : 'border-gray-700 text-gray-300 hover:border-green-500 hover:text-green-400'" 
                @click="role = ''"
              >
                All Users
              </button>
              <button 
                class="px-4 py-2 rounded-lg border transition-all duration-200 text-sm font-medium"
                :class="role === 'listener' ? 'bg-green-600 border-green-600 text-white' : 'border-gray-700 text-gray-300 hover:border-green-500 hover:text-green-400'" 
                @click="role = 'listener'"
              >
                Listeners
              </button>
              <button 
                class="px-4 py-2 rounded-lg border transition-all duration-200 text-sm font-medium"
                :class="role === 'artist' ? 'bg-green-600 border-green-600 text-white' : 'border-gray-700 text-gray-300 hover:border-green-500 hover:text-green-400'" 
                @click="role = 'artist'"
              >
                Artists
              </button>
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            <thead class="bg-gray-800/50 border-b border-gray-700">
              <tr>
                <th class="text-left py-4 px-6 text-sm font-semibold text-gray-300">User</th>
                <th class="text-left py-4 px-6 text-sm font-semibold text-gray-300">Email</th>
                <th class="text-left py-4 px-6 text-sm font-semibold text-gray-300">Role</th>
                <th class="text-left py-4 px-6 text-sm font-semibold text-gray-300">Status</th>
                <th class="text-right py-4 px-6 text-sm font-semibold text-gray-300">Actions</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-800">
              ...
              <tr v-for="u in users" :key="u.id" class="hover:bg-gray-800/30 transition-colors duration-150">
                <td class="py-4 px-6">
                  <div class="flex items-center gap-3">
                    <div class="relative">
                      <img 
                        :src="u.profile_pic || '/placeholder.svg?height=40&width=40'" 
                    ...
                    <div>
                      <div class="font-medium text-white">{{ u.user_name }}</div>
                      <div v-if="u.artist_name" class="text-sm text-green-400">{{ u.artist_name }}</div>
                    </div>
                  </div>
                </td>
                ...
                <td class="py-4 px-6">
                  <span 
                    ...
                    :class="u.role === 'artist' ? 'bg-purple-900/50 text-purple-300 border border-purple-700' : 'bg-blue-900/50 text-blue-300 border border-blue-700'"
                  >
                    {{ u.role }}
                  </span>
                </td>
    ...
    <div v-if="profileOpen" class="fixed inset-0 z-50 overflow-hidden">
      <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" @click="profileOpen = false"></div>
      <div class="absolute right-0 top-0 h-full w-full max-w-md bg-gray-900 border-l border-gray-800 shadow-2xl">
          ...
          <div class="flex-1 overflow-y-auto p-6">
            ...
            <div v-else class="space-y-6">
              ...
              <div class="text-center">
                <img 
                  :src="selected.profile_pic || '/placeholder.svg?height=80&width=80'" 
                  class="w-20 h-20 rounded-full object-cover mx-auto border-4 border-green-500" 
                  :alt="selected.user_name"
                />
                <h3 class="text-xl font-bold text-white mt-3">{{ selected.user_name }}</h3>
                ...
                <span 
                  ...
                  :class="selected.role === 'artist' ? 'bg-purple-900/50 text-purple-300 border border-purple-700' : 'bg-blue-900/50 text-blue-300 border border-blue-700'"
                >
                  {{ selected.role }}
                </span>
              </div>
              ...
              <div class="bg-gray-800/50 rounded-lg p-4 space-y-3">
                ...
                <div class="grid grid-cols-1 gap-3 text-sm">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="text-white">{{ new Date(selected.created_at).toLocaleDateString() }}</span>
                  </div>
                  <div class="flex justify-between">
              ...
              <!-- Artist Info -->
              <div v-if="selected.role === 'artist'" class="bg-gray-800/50 rounded-lg p-4 space-y-3">
                <h4 class="font-semibold text-white">Artist Information</h4>
                <div class="space-y-3 text-sm">
                  <div>
                    <span class="text-gray-400 block">Artist Name:</span>
                    <span class="text-white">{{ selected.artist_name || '—' }}</span>
                  </div>
                  <div>
                    <span class="text-gray-400 block">Description:</span>
                    <span class="text-white">{{ selected.description || 'No description provided' }}</span>
                  </div>
                </div>
              </div>
              ...
              <!-- Actions -->
              <div class="space-y-3">
                <button class="w-full bg-green-600 hover:bg-green-700 text-white font-medium py-2.5 px-4 rounded-lg transition-colors duration-200">
                  Send Message
                </button>
                <button class="w-full border border-gray-700 hover:bg-gray-800 text-gray-300 hover:text-white font-medium py-2.5 px-4 rounded-lg transition-all duration-200">
                  View Activity
                </button>
                <button class="w-full border border-red-700 hover:bg-red-900/50 text-red-400 hover:text-red-300 font-medium py-2.5 px-4 rounded-lg transition-all duration-200">
                  Suspend User
                </button>
              </div>
            </div>
...
async function fetchUsers() {
  ...
  try {
    const { data } = await apiClient.get('api/admin/users', {
      params: { q: q.value, role: role.value, page: page.value, limit: limit.value }
    })
