...
const playFunc = () => {
  ...
  if (currentTrack.value) {
    ...
    if (
      collection.value &&
      collection.value.tracks &&
      collection.value.tracks.length > 0
    ) {
...
onUnmounted(() => {
  ...
  if (window.albumViewCleanup) {
    window.albumViewCleanup();
    delete window.albumViewCleanup;
  }
});
...
<template>
  <div class="p-8 overflow-x-hidden">
    ...
    <div v-else-if="collection">
      ...
      <button
        @click="goBack"
        class="flex items-center gap-2 text-gray-300 hover:text-white transition-colors duration-200 mb-6 group"
      >
        <svg
      ...
      <div class="flex items-center w-full relative h-full">
        <img
          ...
          :src="`${fileServerBaseUrl}/public/images/${
            collection.albumCover ||
            collection.playlistCover ||
            'default_album.png'
          }`"
        />
