  ...
  <div class="bg-gray-800/50 p-6 rounded-lg shadow-md">
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex items-center gap-2">
        <!-- Time filter  -->
        <div
          class="bg-neutral-900 border border-neutral-800 rounded-lg p-1 flex"
        >
          <button
            class="px-3 py-1 rounded-md text-sm"
            :class="
              range === 'today' ? 'bg-neutral-800 text-white' : 'text-gray-300'
            "
          ...
          <button
            class="px-3 py-1 rounded-md text-sm"
            :class="
              range === '7d' ? 'bg-neutral-800 text-white' : 'text-gray-300'
            "
            @click="setRange('7d')"
          >
            7d
          ...
          <button
            class="px-3 py-1 rounded-md text-sm"
            :class="
              range === '30d' ? 'bg-neutral-800 text-white' : 'text-gray-300'
            "
            @click="setRange('30d')"
          >
            30d
          ...
          <button
            class="px-3 py-1 rounded-md text-sm"
            :class="
              range === '' ? 'bg-neutral-800 text-white' : 'text-gray-300'
            "
            @click="setRange('')"
          >
            All
        ...
        />
        <button
          class="px-3 py-2 bg-neutral-800 border border-neutral-700 rounded-lg text-sm text-white"
          @click="load(true)"
        >
          Search
        </button>
    ...
    <ul v-else class="space-y-3">
      ...
      >
        <span
          ...
          :class="{
            'bg-green-500':
              a.type === 'user_signup' || a.type === 'album_created',
            'bg-blue-500':
              a.type === 'song_upload' || a.type === 'artist_added',
        ...
        <div class="flex-1">
          <div class="text-sm text-white">
            ...
            <span
              v-if="a.status"
              class="ml-2 text-xs px-2 py-0.5 rounded bg-neutral-800 border border-neutral-700"
              >{{ a.status }}</span
            >
          </div>
